package com.anassdevops.quiz.security.filter;
...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if(request.getServletPath().equals("/refreshToken")) {
            ...
            String authorizationToken = request.getHeader(SecurityUtil.AUTH_HEAD);
            if(authorizationToken != null && authorizationToken.startsWith(SecurityUtil.PREFIX)) {

                try {
                    String jwt = authorizationToken.substring(SecurityUtil.PREFIX.length());
